...
type Status = "loading" | "joining" | "success" | "error" | "already-member";
...
export default function JoinPage({ params }: PageProps) {
  ...
  const joinRoom = useCallback(async () => {
    setStatus("joining");
    try {
      const res = await fetch("/api/rooms/join-invite-link", {
        method: "POST",
        credentials: "include",
  ...
  useEffect(() => {
    if (isPending || !session?.user || joinAttempted.current) return;
    joinAttempted.current = true;
    // eslint-disable-next-line react-hooks/set-state-in-effect
    joinRoom();
  }, [session?.user, isPending, joinRoom]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Loading */}
          {(status === "loading" || status === "joining" || isPending) && (
            <>
              <div
                className="spinner"
